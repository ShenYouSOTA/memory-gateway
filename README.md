...
- `uv.lock` — 依赖锁定文件
- `Dockerfile` — 容器配置
- `.env.example` — 环境变量配置示例（可选，方便本地开发）
...
```
ai-memory-gateway/
├── main.py                    # 网关主程序（~3000 行）
├── database.py                # 数据库操作（PostgreSQL，~1800 行）
...
**前置条件：** Python 3.12+，推荐使用 [uv](https://docs.astral.sh/uv/) 作为包管理器。
...
# 克隆仓库
git clone https://github.com/your-username/ai-memory-gateway.git
...
# 运行（纯转发模式，不需要数据库）
API_KEY=sk-xxx API_BASE_URL=https://openrouter.ai/api/v1/chat/completions uv run python main.py

# 运行（带记忆功能，需要 PostgreSQL）
DATABASE_URL=postgresql://... MEMORY_ENABLED=true API_KEY=sk-xxx uv run python main.py
```

> 💡 **提示：** 项目根目录有 `.env.example` 文件，包含所有可用的环境变量和说明。可以复制为 `.env` 文件进行本地开发：`cp .env.example .env`
...
> 详细的待办事项和未来规划见 [`docs/todo.md`](docs/todo.md)
