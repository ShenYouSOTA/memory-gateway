...
async def save_message(
    session_id: str, role: str, content: str, model: str = "", metadata: str = None
):
    pool = await get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
...
async def update_last_assistant_message(
    session_id: str, new_content: str, model: str = ""
):
            ...
            results.append(
                {
                    ...
                    "first_time": r["first_time"].isoformat()
                    if r["first_time"]
                    else None,
...
async def search_memories(query: str, limit: int = 10):
    ...
    async with pool.acquire() as conn:
        ...
        if results:
            ...
            for r in results[:3]:
                print(
                    f"   📌 [score={r['score']:.3f}] (hits={r['hit_count']}, imp={r['importance']}) {r['content'][:60]}..."
                )
    ...
    async with pool.acquire() as conn:
        ...
        if query_embedding:
            ...
            sem_passed = sum(
                1
                for r in sem_rows
                if float(r["similarity"]) >= MEMORY_SEMANTIC_THRESHOLD
            )
            sem_max = max((float(r["similarity"]) for r in sem_rows), default=0)
            if sem_total > 0 and sem_passed == 0:
                print(
                    f"   🔢 向量路: {sem_total}条候选全被阈值过滤（最高sim={sem_max:.3f}, 阈值={MEMORY_SEMANTIC_THRESHOLD}）"
                )
            elif sem_total > 0:
                print(
                    f"   🔢 向量路: {sem_passed}/{sem_total}条通过阈值（最高sim={sem_max:.3f}）"
                )
        ...
        # ---- 归一化 + 加权 ----
        kw_norm = _min_max_normalize(
            {mid: v["kw_score"] for mid, v in candidates.items()}
        )
        ...
        if results:
            ...
            print(
                f"🔍 {mode_tag}搜索 '{query}' → {kw_tag} → 命中 {len(results)} 条"
                + (f"（过滤 {filtered} 条低分）" if filtered else "")
            )
            for r in results[:3]:
                print(
                    f"   📌 [score={r['score']:.3f}] (kw={r['hit_count']}, sim={r['similarity']:.2f}, imp={r['importance']}) {r['content'][:60]}..."
                )
            ...
            print(
                f"🔍 混合搜索 '{query}' → 无结果"
                + (f"（{filtered} 条被过滤）" if filtered else "")
            )
...
async def backfill_memory_embeddings(batch_size: int = 20):
    ...
    print(
        f"✅ 本批补算完成：{total_updated}/{len(rows)} 条成功"
        + (f"，剩余 {remaining} 条待处理" if remaining > 0 else "")
    )
...
async def get_all_memories_detail(
    limit: int = None, layer: int = None, active_only: bool = None
):
    """获取所有记忆（含 id，用于管理页面）
...
async def update_memory(memory_id: int, content: str = None, importance: int = None):
    ...
    async with pool.acquire() as conn:
        if content is not None and importance is not None:
            ...
            await conn.execute(
                "UPDATE memories SET content = $1 WHERE id = $2", content, memory_id
            )
...
async def save_session_cache_state(
    session_id: str, summary_parts: list, a_start_round: int
):
    import json
...
async def get_conversations_paginated(page: int = 1, per_page: int = 20):
    ...
    async with pool.acquire() as conn:
        ...
        for r in rows:
            ...
            title = (
                r["custom_title"]
                or (preview[:30] + "..." if len(preview) > 30 else preview)
                or r["session_id"]
            )
            results.append(
                {
...
async def delete_conversation(session_id: str):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "DELETE FROM conversation_titles WHERE session_id = $1", session_id
        )
        await conn.execute(
...
async def batch_delete_conversations(session_ids: list):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "DELETE FROM conversation_titles WHERE session_id = ANY($1)", session_ids
        )
        await conn.execute(
            "DELETE FROM session_cache_state WHERE session_id = ANY($1)", session_ids
        )
...
async def get_fragments_by_date(event_date):
    ...
    start_utc = datetime(
        event_date.year, event_date.month, event_date.day, tzinfo=local_tz
...
async def get_fragments_by_date_range(start_date, end_date):
    ...
    end_utc = datetime(
        end_date.year, end_date.month, end_date.day, tzinfo=local_tz
    ).astimezone(dt_timezone.utc) + timedelta(days=1)
...
async def create_event_memory(
    title: str, content: str, importance: int, event_date, merged_from: list
):
    """创建事件记忆（从碎片合并而来）"""
    pool = await get_pool()
    async with pool.acquire() as conn:
...
async def merge_memories(
    memory_ids: list, new_title: str, new_content: str, importance: int, layer: int = 2
):
    """合并多条记忆为一条新记忆"""
    if not memory_ids:
        return None
...
async def update_memory_with_layer(
    memory_id: int,
    content: str = None,
    importance: int = None,
    title: str = None,
