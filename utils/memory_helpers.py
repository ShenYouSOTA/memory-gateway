...
def _format_memory_lines(memories, timezone_hours):
    """格式化记忆为带日期的文本行"""
    memory_lines = []
    for mem in memories:
        date_str = ""
...
async def build_system_prompt_with_memories(
    ...
    try:
        memories = await memory_service.search_memories(user_message, limit=max_memories_inject)

        if not memories:
            return system_prompt

        memory_text = "\n".join(_format_memory_lines(memories, timezone_hours))
    ...
    try:
        ...
        if skip_conversation_log:
            ...
            for tm in tool_messages:
                ...
                await conversation_service.save_message(session_id, "tool", tm.get("content", ""), model, metadata=meta)
            ...
            if assistant_msg or assistant_tool_calls:
                ...
                await conversation_service.save_message(session_id, "assistant", assistant_msg or "", model, metadata=ast_meta)
        else:
            # 普通对话或首次工具调用
            ast_meta_dict = {}
            if assistant_tool_calls:
            ...
            if assistant_tool_calls:
                await conversation_service.save_message(session_id, "user", user_msg, model)
                await conversation_service.save_message(session_id, "assistant", assistant_msg or "", model, metadata=assistant_meta)
            else:
                # 纯文字对话：re-roll检测
                last_user = await conversation_service.get_last_user_content(session_id)
                if last_user and last_user.strip() == user_msg.strip():
                    updated = await conversation_service.update_last_assistant_message(session_id, assistant_msg)
                    if not updated:
                        await conversation_service.save_message(session_id, "user", user_msg, model)
                        await conversation_service.save_message(session_id, "assistant", assistant_msg, model, metadata=assistant_meta)
                else:
                    await conversation_service.save_message(session_id, "user", user_msg, model)
                    await conversation_service.save_message(session_id, "assistant", assistant_msg, model, metadata=assistant_meta)
        ...
        # 3. 获取已有记忆
        existing = await memory_service.get_recent_memories(limit=80)
        ...
        for mem in filtered_memories:
            await memory_service.save_memory(content=mem["content"], importance=mem["importance"], source_session=session_id)
